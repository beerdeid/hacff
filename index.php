<?php
  ...
  // Pengkondisian Ukuran File
  function convert_filesizeff($bytesff, $decimalsff = 2){
    $ukurancff = array(' B',' KB',' MB',' GB',' TB',' PB',' EB',' ZB',' YB');
    $factorff = floor((strlen($bytesff) - 1) / 3);
    return sprintf("%.{$decimalsff}f", $bytesff / pow(1024, $factorff)) . @$ukurancff[$factorff];
  }
  function convert_filesizeffmax($bytesffmax, $decimalsffmax = 2){
    $ukurancffmax = array(' B',' KB',' MB',' GB',' TB',' PB',' EB',' ZB',' YB');
    $factorffmax = floor((strlen($bytesffmax) - 1) / 3);
    return sprintf("%.{$decimalsffmax}f", $bytesffmax / pow(1024, $factorffmax)) . @$ukurancffmax[$factorffmax];
  }
  ...
  // Output File Size
  $outputcff = convert_filesizeff($ukurancff);
  $outputcffmax = convert_filesizeffmax($ukurancffmax);
?>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="text-center mb-3">
              <img id="gambar" src="assets/img/screenshot/1.jpg" alt="gambar0.1"  />
            </div>
            <div id="ganjil">Game : FreeFire</div>
            <div id="genap">Versi : <a class="text-primary" id="versi"><?= $versiff ?></a></div>
            <div id="ganjil">Status : <img src="https://shields.io/badge/update-success-success?style=flat" alt="update" height="22px" /></div>
            <div id="genap">Developer : (BeerdeMods)</div>
            <div id="ganjil">Kategori : Config - antena tangan - FF</div>
            <div id="genap">Item : Data Config + Data Original</div>
            <div id="ganjil">Ukuran : <?= $outputcff ?></div>
            <div id="genap">Jalur : storage/emulated/0/Android/data</div>
            <div class="text-center mt-4 bawah-3">
              <?= '<a href="'.$fileff.'" class="dwn" id="pencet1"><img src="assets/img/download.png" alt="download" width="25px" /> Unduh</a>' ?>
            </div>
          </div>
          <div class="col-md-6">
            <div class="text-center mb-3">
              <img id="gambar" src="assets/img/screenshot/2.jpg" alt="gambar0.2"  />
            </div>
            <div id="ganjil">Game : FreeFire MAX</div>
            <div id="genap">Versi : <a class="text-primary" id="versi"><?= $versiffmax ?></a></div>
            <div id="ganjil">Status : <img src="https://shields.io/badge/update-success-success?style=flat" alt="update" height="22px" /></div>
            <div id="genap">Developer : (BeerdeMods)</div>
            <div id="ganjil">Kategori : Config - antena tangan - FFMAX</div>
            <div id="genap">Item : Data Config + Data Original</div>
            <div id="ganjil">Ukuran : <?= $outputcffmax ?></div>
            <div id="genap">Jalur : storage/emulated/0/Android/data</div>
            <div class="text-center mt-4 bawah-3">
              <?= '<a href="'.$fileffmax.'" class="dwn" id="pencet2"><img src="assets/img/download.png" alt="download" width="25px" /> Unduh</a>' ?>
            </div>
          </div>
        </div>
      </div>
